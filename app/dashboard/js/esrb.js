    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            if (output[0] == "Éxito") {
                ...
                $('#nuevaClasificacion').modal('close');
...
$( "#frmUpdateRegEsrb" ).submit(function( event ) {
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            if (output[0] == "Éxito") {
                //si la operacion fue un exito, cerramos el modal
                $('#actualizarClasificacion').modal('close');
                //refrescamos la pagina
                attach("esrb", 1);
            }
            console.log(result);
...
$("table").on('click', ".edit", function () {
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            $("#esrbUName").val($data.name);
            console.log("state"+$data.state);
            if($data.state==="1"){
                $("#esrbStateA").prop("checked", true);
            }else{
                $("#esrbStateI").prop("checked", true);
            }

            //seteamos el id (OJO AGEREGAR INPUT TYPE HIDDEN PARA ALMACENAR EL ID)
            $("#esrbId").val($data.id);
            //al setear todos los datos abrimos el modal
            $("#actualizarClasificacion").modal('open');
        }
    });
});
